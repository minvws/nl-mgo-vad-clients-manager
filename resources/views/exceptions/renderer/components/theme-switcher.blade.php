<script nonce="{!! csp_nonce() !!}">
    ...
    (function () {
        ...
        setDarkClass = () => {
            ...
            if (isDark) {
                document.head.insertAdjacentHTML('beforeend', `<style data-theme="dark" nonce="{!! csp_nonce() !!}">${darkStyles}</style>`)
            } else {
                document.head.insertAdjacentHTML('beforeend', `<style data-theme="light" nonce="{!! csp_nonce() !!}">${lightStyles}</style>`)
            }
        }
...
>
    ...
    <div
        x-show="menu"
        class="absolute right-0 z-10 flex origin-top-right flex-col rounded-md bg-white shadow-xl ring-1 ring-gray-900/5 dark:bg-gray-800 invisible"
        @click="menu = false"
    >
        <button
